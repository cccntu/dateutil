...
class tzres(object):
    ...
    def load_name(self, offset):
        ...
        >>> from bs_dateutil.tzwin import tzres
        >>> tzr = tzres()
...
class tzwin(tzwinbase):
    ...
    These are similar to :py:class:`bs_dateutil.tz.tzrange` objects in that
...
class tzwinlocal(tzwinbase):
    ...
    While :class:`bs_dateutil.tz.tzlocal` makes system calls (via the :mod:`time`
    module) to retrieve time zone information, ``tzwinlocal`` retrieves the
    rules directly from the Windows registry and creates an object like
    :class:`bs_dateutil.tz.tzwin`.

    Because Windows does not have an equivalent of :func:`time.tzset`, on
    Windows, :class:`bs_dateutil.tz.tzlocal` instances will always reflect the
    time zone settings *at the time that the process was started*, meaning
    changes to the machine's time zone settings during the run of a program
    on Windows will **not** be reflected by :class:`bs_dateutil.tz.tzlocal`.

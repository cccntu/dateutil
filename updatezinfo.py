...
try:
    ...
    print("bs_dateutil not installed locally, adding src to Python path")
    import sys
...
def main(metadata_file):
    ...
    if not os.path.isfile(metadata["tzdata_file"]):
        ...
        for ii, releases_url in enumerate(releases_urls):
            ...
            try:
                request.urlretrieve(
                    os.path.join(releases_url, metadata["tzdata_file"]),
    ...
    rebuild.rebuild(
        metadata["tzdata_file"], zonegroups=metadata["zonegroups"], metadata=metadata
    )
    print("Done.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "metadata", metavar="METADATA_FILE", default=METADATA_FILE, nargs="?"
    )
